"""
*** SINGLE NEURON LANGUAGE DETECTOR - vers. 2 ***
...
3) NEURONE: una funzione che associa gli input neurali e genera un output compreso tra 0 e 1
    ...
    Il peso dei vari input sull'output è stato calibrato in base a test empirici.
    Il neurone singolo tende a generare falsi negativi se i testi sono brevi, quindi vengono introdotti compensi 
    quando il numero delle parole inserite è inferiore a 10 ma ci sono indicatori linguistici forti (value2 o value3 ≥ 0.7).

    Inoltre, un bias viene aggiunto per spingere in alto solo i casi borderline già promettenti ma solo sopra una certa soglia (output >= 0.6) 
    per evitare di alzare troppo i valori deboli.


            
** CONFRONTO CON LA vers.1 **

La vers.2 performa meglio della vers.1 nei test effettuati. 
Il miglioramento riguarda la diminuzione dei falsi negativi e dei casi di indecisione.
...
while True:
    ...
    print(f"\nLa lunghezza media delle parole inserite é {avg_len:.2f}. Pertanto il valore quarto input neurale é {value4}")
    ...
    print(f"\nNel testo inserito ci sonon {accented_found} parole con accenti. Pertanto il valore del quinto input neurale é {value5}")
